...
const CommericalClass = () => {
  const [json, setJson] = useState("");
  const [file, setFile] = useState("");
  const [err, setErr] = useState("");
  const [load, setLoad] = useState(false);
  const API_URI = "/api/edu/comm/class/export/csv";
  ...
  const handleSubmit = async (e) => {
    ...
    setLoad(true);
    try {
      const response = await fetch("/api/edu/comm/class", {
        method: "POST",
        body: formData,
      });
...
export default CommericalClass;
