...
const PrePrimaryStudents = () => {
  const [json, setJson] = useState("");
  const [file, setFile] = useState("");
  const [err, setErr] = useState("");
  const [load, setLoad] = useState(false);
  const API_URI = "/api/edu/preprimary/stu/export/csv";
  ...
  const handleSubmit = async (e) => {
    ...
    setLoad(true);
    try {
      const response = await fetch("/api/edu/preprimary/stu", {
        method: "POST",
        body: formData,
      });
    ...
    <div style={{ padding: "8px", border: "1px solid gray" }}>
      {load ? (
        ...
        >
          ...
          >
            <a
              href={`${API_CSV}/edu/students/${json}`}
              download={json}
              target="_self"
...
export default PrePrimaryStudents;
