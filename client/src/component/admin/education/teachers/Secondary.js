...
const Secondary = () => {
  ...
  const API_URI = "/api/edu/secondary/teacher/export/csv";
  ...
  const handleSubmit = async (e) => {
    ...
    setLoad(true);
    try {
      const response = await fetch("/api/edu/secondary/teacher", {
        method: "POST",
        body: formData,
      });
    ...
    <div style={{ padding: "8px", border: "1px solid gray" }}>
      {load ? (
        ...
        >
          ...
          >
            عدد المدرسين المرحلة الاعدادية
          </Typography>
          <ButtonMaterial
            variant="contained"
            component="label"
          ...
          >
            <a
              href={`${API_CSV}/edu/teachers/${json}`}
              download={json}
              target="_self"
