...
const Primary = () => {
  ...
  const API_URI = "/api/edu/azhar/primary/export/csv";
  ...
  const handleSubmit = async (e) => {
    ...
    setLoad(true);
    try {
      const response = await fetch("/api/edu/azhar/primary", {
        method: "POST",
        body: formData,
      });
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = "عدد_الطلاب_المرحلة_الابتدائية_الازهر.csv";
    ...
    <div style={{ padding: "8px", border: "1px solid gray" }}>
      {load ? (
        ...
        >
          ...
          >
            عدد الطلاب المرحلة الابتدائية الازهر
          </Typography>
          <ButtonMaterial
            variant="contained"
            component="label"
