...
const Secondary = () => {
  ...
  const API_URI = "/api/edu/azhar/secondary/export/csv";
  ...
  const handleSubmit = async (e) => {
    ...
    setLoad(true);
    try {
      const response = await fetch("/api/edu/azhar/secondary", {
        method: "POST",
        body: formData,
      });
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = "عدد_الطلاب_الاعدادية_الازهر.csv";
    ...
    <div style={{ padding: "8px", border: "1px solid gray" }}>
      {load ? (
        ...
        >
          ...
          >
            عدد الطلاب المرحلة الاعدادية الازهر
          </Typography>
          <ButtonMaterial
            variant="contained"
            component="label"
