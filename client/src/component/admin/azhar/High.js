...
const High = () => {
  ...
  const API_URI = "/api/edu/azhar/high/export/csv";
  ...
  const handleSubmit = async (e) => {
    ...
    setLoad(true);
    try {
      const response = await fetch("/api/edu/azhar/high", {
        method: "POST",
        body: formData,
      });
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = "عدد_طلاب_الثانوي_الازهر.csv";
    ...
    <div style={{ padding: "8px", border: "1px solid gray" }}>
      {load ? (
        ...
        >
          ...
          >
            عدد الطلاب المرحلة الثانوية الازهر
          </Typography>
          <ButtonMaterial
            variant="contained"
            component="label"
