...
const Kids28 = () => {
  ...
  const API_URI = "/api/health/death/kids/under28/export/csv";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/health/death/kids/under28",
        {
          method: "POST",
          body: formData,
        }
    ...
    <div style={{ padding: "8px", border: "1px solid #807040" }}>
      {load ? (
        ...
        >
          ...
          >
            <a
              href={`${API_CSV}/health/kids_28/${json}`}
              download={json}
              target="_self"
