const Secomdary = require("../../../models/edu/teachers/Secomdary");
var fs = require("fs");
...
const getTeachers = async (req, res) => {
  try {
    const teachers = await Secomdary.aggregate([
      {
        $group: {
          _id: {
            المديرية: "$المديرية",
...
const importData = async (req, res) => {
  try {
    ...
    await Secomdary.insertMany(result["undefined-0"]);
...
const exportToCsv = async (req, res) => {
  try {
    ...
    const data = await Secomdary.find({});
    //console.log(data);
    ...
    mongoXlsx.mongoData2Xlsx(
      ...
      model,
      {
        fileName: "عدد_المدرسين_الاعدادي" + Date.now() + ".csv",
