const Community = require("../../../models/edu/schools/Community");
...
const getSchools = async (req, res) => {
  try {
    const schools = await Community.aggregate([
      {
        $group: {
          _id: {
            السنة: "$السنة",
...
const importData = async (req, res) => {
  try {
    ...
    await Community.insertMany(result["undefined-0"]);
...
const exportToCsv = async (req, res) => {
  try {
    ...
    mongoXlsx.mongoData2Xlsx(
      ...
      model,
      {
        fileName: "عدد_المدارس_التعليم_المجتمعي" + Date.now() + ".csv",
